...
void ProcessBuySetup(string symbol, int entryTf, datetime candleTime, TrendInfo &trend, PullbackInfo &pullback)
{
   ...
   double stopReference = GetSetupStopReference(symbol, OP_BUY, entryStopReference, pullback);
...
void ProcessSellSetup(string symbol, int entryTf, datetime candleTime, TrendInfo &trend, PullbackInfo &pullback)
{
   ...
   double stopReference = GetSetupStopReference(symbol, OP_SELL, entryStopReference, pullback);
...
void ValidateH4Pullback(string symbol, TrendInfo &trend, PullbackInfo &pullback)
{
   ...
   if(trend.direction > 0)
   {
      ...
      double minPullbackPips = H4MinPullbackPips;
   ...
   if(trend.direction < 0)
   {
      ...
      double minBearishPullbackPips = H4MinPullbackPips;
...
bool ValidateH4TrendRejection(string symbol, int orderType, string &reason)
{
   ...
   double buffer = H4RejectionBreakBufferPips * PipSize(symbol);
...
double GetSetupStopReference(string symbol, int orderType, double entryStopReference, PullbackInfo &pullback)
{
...
void CalculateSuggestedLevels(string symbol, int orderType, double triggerLevel, double marketPrice, double stopReference, double targetReference, LevelInfo &levels)
{
   ...
   double buffer = StopBufferPips * PipSize(symbol);
   ...
   double riskPips = levels.risk / PipSize(symbol);
   double maxRiskPips = MaxRiskPips;
   if(UseMaxRiskDistanceFilter && maxRiskPips > 0.0 && riskPips > maxRiskPips)
   {
      levels.reason = "Risk distance " + DoubleToString(riskPips, 1) + " pips exceeds allowed " + DoubleToString(maxRiskPips, 1) + ".";
      return;
   }
